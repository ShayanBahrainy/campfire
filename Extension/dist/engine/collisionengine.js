...
export class CollisionEngine {
    constructor(mapFactor) {
        this.MAP_FACTOR = mapFactor;
    }
    static closestVertexToCircle(object, circle) {
        let points;
        let circleCenter = new Point(circle.x, circle.y);
        ...
        let minDistance = points[0].distance(circleCenter) - circle.radius;
    ...
    static getAxes(object, otherShape) {
        let segments = [];
        let axes = [];
        if (object.shape == "rectangle") {
            const rectangle = object;
        ...
        if (object.shape == "polygon") {
            const polygon = object;
            let points = Renderer.calculatePoints(polygon.vertexes, polygon.apothem, object.x, object.y, object.rotation);
            segments.push(new LineInert(points[points.length - 1], points[0]));
            for (let i = 1; i < points.length; i++) {
                segments.push(new LineInert(points[i - 1], points[i]));
            }
    ...
    SAT(first, second) {
        ...
        if (first.shape == "circle" && second.shape == "circle") {
            const circle_one = first;
            const circle_second = second;
